...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  private async * request(
    ...
    while (true) {
      ...
      // STREAM_CLOSED attribution (ADR: 上游断流归因): pin the transport
      // boundary (provider host, HTTP status, model) into the parser's
      // diagnostics so abnormal terminations carry their origin.
      let providerHost = connection.baseURL
      try {
        providerHost = new URL(connection.baseURL).host
      } catch {
        // 非 URL 形态的 baseURL 保持原样
      }
      const diagContext = `provider=${providerHost} status=${response.status}`
      try {
        yield* translate(parseSse(response.body, onActivity, { context: diagContext }))
      } catch (error) {
        if (error instanceof LlmError && error.code === 'STREAM_CLOSED') {
          const bodyModel = (body as { model?: unknown } | undefined)?.model
          console.error(`[dsh-llm] provider stream cut: ${diagContext} model=${bodyModel ?? ''}`)
        }
        throw error
      }
      return
