...
describe('api client 401 刷新', () => {
  ...
  it('401 → 单飞 refresh → 重放原请求成功', async () => {
    ...
    expect(store.get('auditmind.refresh_token')).toBe('refresh-2'); // 滚动续签
  });
  ...
  it('refresh 失败 → 清会话（路由守卫接管跳登录）', async () => {
    ...
    try {
      ...
      expect(store.get('auditmind.refresh_token')).toBeUndefined(); // Map.get：缺失即 undefined（clearSession 已执行）
