...
export default function FindingsPage({ taskId }: { taskId: string }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['findings', taskId, cursor, verdictFilter, localFilter],
    queryFn: async () =>
      (await api.get('/v1/findings', {
        params: {
          task_id: taskId,
          pagination: { page_size: 100, cursor },
          ...(verdictFilter ? { filter: { ai_verdict: verdictFilter } } : {}),
        },
      ...
      <Space style={{ marginBottom: 12 }}>
        ...
        <Select
          ...
          onChange={(v) => {
            ...
            else { setLocalFilter('all'); setVerdictFilter(''); }
          }}
