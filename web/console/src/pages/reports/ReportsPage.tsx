...
const FORMAT_LABEL: Record<number, string> = { 1: 'PDF', 2: 'HTML', 3: 'JSON', 4: 'CSV' };
const fmtLabel = (f: number) => FORMAT_LABEL[f] ?? '—'; // 0/历史未记录 → —（不显示"未知"误导）
            ...
            { title: '生成时间', dataIndex: 'generated_at', render: (v: string | null) => (v ? dayjs(v).format('YYYY-MM-DD HH:mm:ss') : '—') },
            {
              ...
              render: (_: unknown, rec: ReportRow) => (
                <Space>
                  ...
                  <Button size="small" onClick={() => download(rec.report_id)}>下载</Button>
                </Space>
              ),
            },
          ]}
